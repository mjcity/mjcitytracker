...
function drawTileOffice(){
  const w = Math.floor(cv.width / TILE);
  const h = Math.floor(cv.height / TILE);
  for(let y=0;y<h;y++){
    for(let x=0;x<w;x++){
      const isRoom2 = x>17 && y<8;
      const isRoom3 = x>14 && y>10;
  ...
  ctx.fillRect(cv.width-10,0,10,cv.height);
  ctx.fillRect(0,cv.height-10,cv.width,10);
  ctx.fillRect(420,0,10,220);
  ctx.fillRect(340,250,560,10);
...
function pixelShelf(x,y){
  ctx.fillStyle='#6b4f2d'; ctx.fillRect(x,y,86,20);
  ctx.fillStyle='#4b3621'; ctx.fillRect(x,y+20,86,4);
  ['#ef4444','#22c55e','#3b82f6','#fbbf24'].forEach((c,i)=>{ctx.fillStyle=c;ctx.fillRect(x+8+i*12,y+6,8,10);});
}
function pixelPlant(x,y){
  ctx.fillStyle='#d1d5db'; ctx.fillRect(x,y,14,10);
  ctx.fillStyle='#16a34a'; ctx.fillRect(x+4,y-10,6,10);
  ctx.fillRect(x+1,y-7,4,7); ctx.fillRect(x+9,y-7,4,7);
}
function pixelMachine(x,y){
  ctx.fillStyle='#9ca3af'; ctx.fillRect(x,y,26,38);
  ctx.fillStyle='#374151'; ctx.fillRect(x+5,y+6,16,10);
}
...
function drawPixelAgent(a){
  const px=Math.round(a.x*TILE), py=Math.round(a.y*TILE);
  const frame = Math.floor(tick/14)%2;

  // legs
  ctx.fillStyle='#1f2937';
  if(a.status==='walk'){
    ctx.fillRect(px-5,py-2,3,6); ctx.fillRect(px+2,py-1+frame,3,6);
  } else {
    ctx.fillRect(px-4,py-1,3,6); ctx.fillRect(px+1,py-1,3,6);
  }
